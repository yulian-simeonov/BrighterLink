...
- (void)prepareLayout
{
    ...
    NSAssert(_columnCount > 0, @"columnCount for UICollectionViewWaterfallLayout should be greater than 1.");
...
//    _interitemSpacing = floorf((width - _columnCount * _itemWidth) / (_columnCount - 1));
    ...
    for (NSInteger idx = 0; idx < _itemCount; idx++) {
        ...
        CGFloat columnwidth = [self getColumnWidth:columnIndex];
        
        CGFloat xOffset = [self getOffsetWidthColumn:columnIndex];
...
{
    if(self.aryWidthsForColumns == nil || self.aryWidthsForColumns.count <= column )
    {
        return self.itemWidth;
    }
    
    CGFloat width = [[self.aryWidthsForColumns objectAtIndex:column] floatValue];
    
    return width;
}

- (CGFloat) getOffsetWidthColumn:(NSInteger)column
{
    CGFloat xOffset = _sectionInset.left;
    for (int n = 0 ; n < column; n ++) {
        xOffset += ([self getColumnWidth:n] + _interitemSpacing);
    }
